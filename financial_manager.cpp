...
public:
    void addTransaction(const string& name, double amt, char type, const string& date) {
        ...
        sort(transactions.begin(), transactions.end(),
            [](const Transaction& a, const Transaction& b) { return a.transactionDate < b.transactionDate; });
    }

    void printJournalToFile(const string& fileName) const {
        ofstream outFile(fileName);
        if (outFile.is_open()) {
            outFile << left << setw(12) << "Date" << setw(20) << "Particular" << setw(12) << "Debit" << "Credit\n";
            string currentDate;
...
StockData findMaxProfit(int prices[], int start, int end) {
    ...
    if (leftProfit.profit >= rightProfit.profit && leftProfit.profit >= maxCrossingProfit) {
        return leftProfit;
    } else if (rightProfit.profit >= leftProfit.profit && rightProfit.profit >= maxCrossingProfit) {
        return rightProfit;
    } else {
...
int main() {
    ...
    do {
        ...
        case 3: {
            ...
            cout << "Buy on day " << result.buyDay + 1 << ", sell on day " << result.sellDay + 1
